...
- [x] T004 Añadir `measurement` a `src/config.ts` según `contracts/measurement-events.md` (`ga4Id: ''`, `metaPixelId: ''`, `consentVersion: '2026-09'`, `consentEndpoint` derivado de `EVENTALIST_BACKEND`, `metaDomainVerification: ''`) con comentario que explique que vacíos no cargan nada
...
- [x] T009 [P] Crear `src/measurement/consent.ts` (lectura y escritura de `ConsentState` en `localStorage['tdm.consent']` con `try/catch`, `isAccepted(version)`, `accept()`, `revoke()`, eventos `tdm:consent` y `tdm:revoke`), `src/measurement/attribution.ts` (captura de `utm_*`, `fbclid`, `gclid`, construcción de `fbc`, lectura de `_fbp`/`_fbc`, `gtag('get')` con tope de 2 s, `Attribution` en `localStorage['tdm.attribution']`) y `src/measurement/tags.ts` (Consent Mode, carga de gtag y del píxel, `trackBeginCheckout(value?)` con `eventId` compartido) según `contracts/measurement-events.md` y `contracts/pretix-attribution.md` §1; todo con degradación silenciosa (FR-006)
...
- [x] T017 [P] [US1] Reescribir la casilla obligatoria de Pretix en `docs/pretix-tienda-textos.md` (línea del "Confirmation text") con el texto del dictamen §4 y anotar que la fecha de aplicación en el panel es `ATTRIBUTION_CONSENT_SINCE`; commit propio
