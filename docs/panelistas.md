# Panelistas: fuentes, fotos y pendientes

Investigación pública hecha el 3 de septiembre de 2026 para completar las tarjetas
del sitio sin esperar el formulario. Todo lo publicado está sujeto a corrección por
cada panelista. Aquí va lo necesario para pedir la revisión.

## Qué pedirle a cada panelista

Un solo mensaje sirve para todos:

1. Revisar nombre, credencial y bio tal como aparecen en el sitio.
2. Confirmar o corregir las redes enlazadas (abajo, columna "Redes").
3. Enviar una foto oficial en buena resolución. Las de hoy son de prensa y no
   tenemos licencia para usarlas de forma permanente.

## Tabla de control

| Panelista | Credencial publicada | Redes enlazadas | Foto usada (crédito) | Pendiente de confirmar |
|---|---|---|---|---|
| Daniel Samper Pizano (fundador de Cambio 16 Colombia, no director; corrección de los organizadores 2026-09-07) | Periodista y escritor · Los Danieles | X @DanielSamperPi · Los Danieles · Wikipedia | Foto de autor de Cambio (b/n), derechos reservados. Alternativa con licencia libre: Wikimedia Commons, NotimexTV 2018, CC BY 3.0, pero de baja calidad | La cuenta de X fue señalada como parodia en 2020; en 2026 Los Danieles la etiqueta como suya. Confirmar con él. |
| Darío Restrepo | Periodista · codirector del encuentro | Wikipedia | Foto nueva del 2026-09-14 (original horizontal de 3840 x 2160 px, en color, en un patio con geranios); recorte propio a 4:3 y 960 px, con el encuadre de la de Daniel Samper Pizano (cabeza y hombros). Reemplaza la de prensa de El Tiempo y la vertical que envió él el 2026-09-07. Anotar el crédito y la autorización de uso | **Identidad**: la bio corresponde a Darío Restrepo Vélez (exdirector de Citytv, n. 1949). Ninguna fuente pública lo vincula con el encuentro. Confirmar con Fernando Cordovez antes de publicar. No tiene redes públicas. |
| León Valencia | Director · Fundación Paz y Reconciliación | X @LeonVaLenciaA · autor en pares.com.co · Wikipedia | Presidencia de Colombia, 13 de mayo de 2025, dominio público (Flickr de Presidencia vía Wikimedia Commons, File:León_Valencia.jpg); recorte propio a 4:3 del original horizontal | Añadido el 2026-09-09 por los organizadores para "Surgimiento de las guerrillas". Año de nacimiento sin consenso (1955 en Wikipedia y Planeta; 1956 en La Silla Vacía): no se publica. Salió de Semana en 2017, por eso la bio no lo llama columnista de Semana. Instagram, LinkedIn y Facebook no verificados; no se enlazaron. Informe de fuentes en docs/investigacion-leon-valencia.md. |
| Marisol Gómez | Columnista · Cambio | X @Marisol_GomezG · LinkedIn · Facebook · columna en Cambio | Festival Gabo 2024, sin crédito | Salió de El Tiempo en 2019 y fue concejal de Bogotá 2020-2023. Su LinkedIn está desactualizado. |
| Luz María Sierra | Directora · El Colombiano | X @LuzMaSierra · LinkedIn (probable) | Forbes Colombia 2026, crédito El Colombiano | Sigue siendo directora (no "exdirectora"). Instagram @luzmariasierra no parece suyo; no se enlazó. |
| Martha Soto | Editora de la Unidad Investigativa · El Tiempo | Página de autora en El Tiempo | Retrato de estudio, El Tiempo (vía gerente.com), 683 px, derechos reservados; pedirle foto oficial en alta | Añadida el 2026-09-07 por los organizadores; sin franja en la agenda todavía. Es Martha Elvira Soto Franco. No tiene redes personales verificables; su perfil remite a la X institucional @UinvestigativaET. Informe en docs/investigacion-martha-soto.md. |
| Cecilia Orozco | Columnista · El Espectador | X @CeciliaOrozcoT · columna en El Espectador · Wikipedia | RTVC / Señal Colombia, octubre 2025 | Dejó la dirección de Noticias Uno en marzo de 2024; sigue en la junta. Conduce "Parte y Contraparte" en Señal Colombia. |
| Marta Ruiz | Periodista · excomisionada de la Verdad | X @martaruiz66 (probable) · autora en Cambio | Shift The Power Summit 2023, suministrada por ella | **Grafía**: firma "Marta", sin h. El documento de agenda decía "Martha". |
| Guillermo González | Fundador de Número · columnista de El Espectador | X @guillogonzale12 · LinkedIn (probable) · El Ala de Arriba | Foto oficial enviada por él en el formulario (29/8/2026), con autorización de uso de imagen y de datos | Formulario recibido: confirma credencial, trayectoria y bio. Redes que declara: X y Facebook (solo X está enlazada; pedirle la URL de Facebook). |
| Yolanda Ruiz | Periodista · Simón Bolívar a la Vida y Obra 2025 | X @YolandaRuizCe · Instagram @yolandaruizperiodista · Facebook (probable) · Wikipedia | Fundación Gabo, Lina Botero, Festival Gabo 2025 | Su sitio yolandaruiz.com está caído; no se enlazó. |
| María Elvira Samper | Periodista · exdirectora de Semana y Cambio | X @monasamper · Wikipedia | Contexto Media 2024 (mismo retrato que usan Planeta y Ulibro) | No hay fuente que la vincule a CM& (la ficha anterior lo decía). Dejó RCN Radio en 2022. Sin columna regular hoy. |

## Fotos

Recortadas a 4:3 y 960 px de ancho en `src/assets/speakers/`. Se muestran en blanco y negro con grano
(utilidad `.grain`), así que el color original no importa.

## Cambios de agenda hechos el 3 de septiembre de 2026

- Sin "por confirmar" visibles: los talleres sin hora muestran solo el día; los
  conversatorios con un solo nombre muestran ese nombre; "Surgimiento de las
  guerrillas" quedó sin panelistas hasta el 2026-09-09, cuando entró León Valencia.
- Cóctel de bienvenida sin sede (no está confirmada).
- Se eliminaron las dos tarjetas "Por confirmar" del carrusel.


## Retirados

- Jorge Cardona: retirado el 2026-09-07 por indicación de los organizadores (aún no han hablado con él). Su investigación queda en la sesión por si vuelve.
